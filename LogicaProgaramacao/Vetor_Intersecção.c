...
#define TM 101
main(){
       int a[TM],b[TM],c[TM-1],i,j,lim=0;
       ...
       printf("\nEste eh o vetor c, que representa a interseccao dos vetores a e b:\n\n");
       if(lim!=0){
       for(i=0;i<lim;i++){
           printf("%d ",c[i]);
           }
          ...
          printf("Conjunto vazio ^^!: %c",157); //o caractere aparecerá somente se no printf houver o %c acompanhado do código numérico na tabela ASCII  
       getch();
       }                       
               /* BOM PROVEITO */                                                            
